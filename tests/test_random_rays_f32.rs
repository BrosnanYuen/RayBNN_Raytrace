...
fn test_random_rays() {
    ...
    let mut neuron_pos_cpu:Vec<f32> = vec![ 0.1, -1.7,      -0.1, 0.4,          -0.3, -4.2,        7.5, -0.7,      -0.3,-0.6,           0.1,-0.7,        0.2, -0.7 ];
    ...
    RayBNN_Raytrace::Generate::Random::generate_random_rays_to_center(
        &neuron_pos,
        ray_num,
        con_rad,
    ...
    let mut start_line_act_cpu = vec!(f32::default();start_line_act.elements());
    start_line_act.host(&mut start_line_act_cpu);

    let mut start_line_pred_cpu = vec!(f32::default();start_line.elements());
    start_line.host(&mut start_line_pred_cpu);

    start_line_act_cpu = start_line_act_cpu.par_iter().map(|x|  (x * 1000000.0).round() / 1000000.0 ).collect::<Vec<f32>>();

    start_line_pred_cpu = start_line_pred_cpu.par_iter().map(|x|  (x * 1000000.0).round() / 1000000.0 ).collect::<Vec<f32>>();
    ...
    let ( mut sumdist,_) = arrayfire::sum_all::<f32>(&magsq);
    ...
    let mut neuron_pos_cpu:Vec<f32> = vec![ 0.1, -1.7, 0.3,     -0.1, 0.4, -0.7,          -0.3, -4.2, -3.9,       7.5, -0.7, -2.1,      -0.3,-0.6,0.9,            0.1,-0.7,0.5,         0.2, -0.7, -5.2  ];
    ...
    let mut neuron_pos_cpu:Vec<f32> = vec![ 0.1, -1.7, 0.3, 0.2,          -0.1, 0.4, -0.7, -2.2,         -0.3, -4.2, -3.9, 0.9,         7.5, -0.7, -2.1, 1.2,        -0.3,-0.6,0.9,-0.11,            0.1,-0.7,0.5,-0.21,           0.2, -0.7, -5.2, 0.5 ];
